...
@spare_service_router.get ('/spare-services/by-service_id/{service_id}', tags=['spare-service'], response_model= SpareServiceRequest)
def get_spare_services_by_service_id (service_id:int =Path(ge=1, le=2000)) -> SpareServiceRequest:
    db = Session()
    result = SpareServiceServices(db).get_spare_services_by_service_id(service_id)
    return JSONResponse(status_code=200, content=jsonable_encoder(result))
...
@spare_service_router.put('/spare-service/{spare_service_id}', tags=['spare-service'], response_model=dict, status_code=200)
def update_spare_service(spare_service_id: int, SpareServiceRequest: SpareServiceRequest) -> dict:
    db = Session()
    car_brand_service = SpareServiceServices(db)

    car_brand_service.update_spare_service(spare_service_id, SpareServiceRequest)
    
    return JSONResponse(status_code=201, content={"message": "Se ha modificado el articulo de recambio"})

@spare_service_router.delete('/spare-service/{spare_service_id}', tags=['spare-service'], response_model=dict, status_code=200)
def delete_spare_service(spare_service_id: int) -> dict:
    db = Session()
    car_brand_service = SpareServiceServices(db)
    car_brand_service.delete_spare_service(spare_service_id,)
